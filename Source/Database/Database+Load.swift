...
public extension Database {
    ...
    public func load<T>(_ type: T.Type, where predicate: Predicate<T>) -> [T] {
        return realm.objects(T.ManagedObjectType.self).filter(predicate.toPredicate()).map { T(fromManagedObject: $0) }
    }
    
    public func load<T>(_ type: T.Type, sortBy sortDescriptor: SortDescriptor<T>) -> [T] {
        return realm.objects(T.ManagedObjectType.self).sorted(by: sortDescriptor.toSortDescriptors()).map { T(fromManagedObject: $0) }
    }

    public func load<T>(_ type: T.Type, where predicate: Predicate<T>, sortBy sortDescriptor: SortDescriptor<T>) -> [T] {
